...
- **解析结果**：
  ...
  - `职务职位`：职务职位名称（如"海关业务二级主办及以下职位"）
  - `职位代码`：12位职位代码
- **应用范围**：`/api/search`、`/api/positions/analysis` 均使用解析后的字段

## 数据提取说明
- **入口**：`python -c "from customs_pipeline.extractor import build_csv; build_csv()"`
- **数据来源**：`downloads/2026/*.docx`
- **输出**：`output/2026/2026海关公示汇总.csv`
- **解析逻辑**：
  - 支持一个 docx 中有多个表格（厦门海关等有续表）
  - 后续表格复用第一个表格的列映射
  - 自动跳过空行和无效数据
- **当前数据量**：2576 条（42 个关区）
