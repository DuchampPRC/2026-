...
### 设计理念
采用现代极简设计语言，强调内容优先、交互友好。

### 设计语言
- **配色方案**：青色为主色调 (#0ea5e9)，搭配绿色、紫色、橙色作为辅助色
- **圆角**：使用 6px-24px 圆角，营造柔和视觉
- **阴影**：多层阴影系统 (shadow-sm → shadow-xl)
- **动画**：fade-in、slide-up 等过渡动画，提升交互体验

### 组件系统
- **卡片 (`.card`)**：圆角容器，含标题栏和内容区
- **统计卡片 (`.stat-card`)**：带顶部渐变条，包含图标、数值、标签
- **表格 (`.data-table`)**：斑马条纹、行悬停效果
- **弹窗 (`.modal-*`)**：毛玻璃背景遮罩、居中内容框
- **筛选器 (`.filter-*`)**：圆角输入框、下拉框、按钮

### 布局
- **电脑端**：左侧 260px 侧边栏 + 主内容区
- **平板端**：顶部标签栏导航
- **手机端**：顶部标题栏 + 底部 Tab 导航
